(function() {
  var DEFAULT_PORT, DEFAULT_RATE, DEFAULT_ROOT, DEFAULT_WEBSERVICE_DELAY, DEFAULT_WEBSERVICE_FOLDER, colors, connect, dataTransfer, fileTransfer, fs, http, livepage, ncli, parse, path, start, sys, util, webservice, _fileTransferCallback, _init, _isLiveReload, _isVerbose, _now, _parseCLI, _port, _rate, _root, _router, _server, _version, _versionNumber, _webserviceDelay, _webserviceFolder;
  ...
  */
  ...
  _init = function() {
    ...
    return _server.use(fileTransfer(transferer, _root, _fileTransferCallback, hooks));
  };
  ...
  */
  ...
  start = function() {
    ...
    if (_isVerbose) console.log("   mode ".cyan + "verbose");
    return console.log("------------------------------------------");
  };
